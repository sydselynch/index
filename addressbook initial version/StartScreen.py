...
class Start():
    ...
    def DeletePrompt(self):
        ...
        if (len(self.selection) > 0):
            ...
            yesButton = Button(self.prompt, text= "OK", width=10, command= self.DeleteFile)
            noButton = Button(self.prompt, text="Cancel", width=10, command= self.prompt.destroy)
    ...
    def ExportPrompt(self):
        ...
        if len(fileIndex) != 0 :
            ...
            f.close()
            self.prompt = Toplevel(self.root)
            self.prompt.title(string="Success")
            self.prompt.minsize(width=150, height=75)
            self.prompt.maxsize(width=150, height=75)
            textLabel = Label(self.prompt, text="Export successful!")
            congratButton = Button(self.prompt, text="OK", width=10, command=self.prompt.destroy)
